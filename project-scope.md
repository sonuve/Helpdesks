...
- **Autonomous auto-resolution (first slice of "AI autonomy"):** New tickets are now judged on arrival for whether they can be fully answered with no human at all; if so, the AI sends the reply and resolves the ticket itself, with zero review step — the first real instance of the "AI autonomy" decision above, not just a human-assist tool. It does **not** resolve the "Escalation"/"Confidence threshold" open question below, though: there's no numeric confidence score or per-category threshold, just a conservative binary yes/no judgment (default to "no" whenever unsure), and there's no escalation queue — a ticket judged not resolvable is simply left `OPEN`, exactly as before this existed. See CLAUDE.md's "Ticket auto-resolution".
- **Email ingestion provider:** SendGrid's Inbound Parse webhook — SendGrid receives mail sent to the configured support address and forwards it to our server as an HTTP POST, rather than polling a mailbox via the Gmail API, Microsoft Graph, or generic IMAP. This settles the provider/API half of the "Email ingestion" open question below; reply threading, attachment handling, and spam/phishing filtering are still open. See `server/src/routes/inbound-email.ts`.
...
- **Email ingestion:** Provider is now decided (see "Email ingestion provider" above). Still open: how replies are threaded to existing tickets (Message-ID/References headers vs. subject matching); attachment handling; spam/phishing filtering before content reaches the AI prompt.
