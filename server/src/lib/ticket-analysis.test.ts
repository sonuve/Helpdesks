...
// Mocks the `ai` package's generateText/generateObject directly (rather
// than mock.module-ing this file itself, the way tickets.test.ts does for
// its route tests) so this file can exercise the real
// summarizeTicket/classifyTicket/evaluateAutoResolution — in particular
// evaluateAutoResolution's deterministic sign-off appended after the AI
// call, which tickets.test.ts's whole-module mock never runs. Must be
// registered before importing ./ticket-analysis.js, since that import
// transitively pulls in ./reply-drafting.js, whose top-level `google(...)`
// call and `generateText`/`signReply` this file depends on.
const generateTextMock = mock(async (_options: { system: string; prompt: string }) => ({
  text: "  Raw AI output.  ",
}));
// One mock for generateObject shared between classifyTicket's enum output
// mode (`object` is a plain string) and evaluateAutoResolution's schema
// mode (`object` is `{ resolvable, reply }`) — one mock per external
// dependency, not one per internal caller of it.
...
const { classifyTicket, evaluateAutoResolution, summarizeTicket } =
