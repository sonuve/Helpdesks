...
**Never write the role strings `"ADMIN"` / `"AGENT"` literally — import `Role` from `core` and use `Role.ADMIN` / `Role.AGENT`, in both `client/` and `server/`.** This applies to comparisons (`user.role === Role.ADMIN`), fixture/default values, `<SelectItem value={Role.AGENT}>`, everywhere. `core/src/schemas/user.ts` is the single definition (`export const Role = { ADMIN: "ADMIN", AGENT: "AGENT" } as const` + `export type Role = (typeof Role)[keyof typeof Role]`) — validated in `createUserSchema` with `z.nativeEnum(Role, ...)`.

It's a plain `const` object, not TypeScript's `enum` keyword — deliberately, matching how Prisma itself generates `server/src/generated/prisma/enums.ts`'s `Role` (Prisma stopped emitting real `enum`s; they aren't erasable/isolated-modules-safe, which matters here since both Bun and Vite transpile file-by-file). Keeping `core`'s copy the same shape as Prisma's, rather than using `enum`, means the two stay structurally interchangeable with no casts — a real TS `enum` would be a *nominal* type and wouldn't be. `core` still can't import the Prisma-generated one directly (`client` must never pull in server-only generated code), so the values are declared twice by necessity; keep them in sync if the roles ever change. `server/src/lib/auth.ts` is the one place that intentionally keeps using Prisma's own `Role` rather than `core`'s — it's typing a Prisma-adapter field, so the Prisma-generated type is the correct one there.
